...
list<bar> bergmannHommelOnline(pTHX_ uint _k, AV* _inBars, double _alpha) {
  // P-value matrix
  ...
  for (I32 i = 0; i <= av_len(_inBars); ++i) {
    ...
    if (first < 0 or first >= _k) {
      croak("_inBars[%d][0] = %lu is not between 0 and %u", i, first, _k);
    }
    if (second < 0 or second >= _k) {
      croak("_inBars[%d][1] = %lu is not between 0 and %u", i, second, _k);
    }
