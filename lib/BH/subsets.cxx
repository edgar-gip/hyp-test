...
static void computeSetsRec(const vector<vector<double>>& probs, double alpha,
                           vector<vector<bool>>& list, double currentProb,
                           vector<vector<uint>>& currentSets, uint next);
static double computeProb(const vector<vector<double>>& probs,
                          const vector<uint>& set);
static uint computeSize(const vector<vector<uint>>& sets, uint remain);
static void addToList(vector<vector<bool>>& list,
...
void computeSets(const vector<vector<double>>& probs, double alpha,
...
static double computeProb(const vector<vector<double>>& probs,
  ...
  uint k = set.size() - 1;
  for (uint i = 0; i < k; i++) {
    if (ret > probs[set[i]][set[k]]) {
      ret = probs[set[i]][set[k]];
    }
  }
  return ret;
}
...
static void computeSetsRec(const vector<vector<double>>& probs, double alpha,
  ...
  if (currentProb * currentSize <= alpha) {
    return;  // branch
  }
  if (remain == 0) {
